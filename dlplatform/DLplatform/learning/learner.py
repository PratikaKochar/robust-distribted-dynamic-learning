...
class IncrementalLearner(Learner):
    ...
    def __init__(self, batchSize : int, syncPeriod : int, delta : float, aggType = "average", name = "IncrementalLearner", identifier = ""):
        '''
        Initialize all the parameters with initial values and base class with name IncrementalLearner
    ...
    def obtainData(self, example: tuple):
        ...
        if len(self._trainingBatch) >= self._batchSize:
            ...
            if self._aggType == 'average':
                curDist, localConditionHolds = self.checkLocalConditionHolds()
            elif self._aggType == 'geometeric_median':
                curDist, localConditionHolds = self.checkLocalConditionHoldsGM()
