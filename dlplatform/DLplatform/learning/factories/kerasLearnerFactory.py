...
class KerasLearnerFactory(LearnerFactory):
    ...
    '''
    def __init__(self, network : KerasNetwork, updateRule, learningRate, lossFunction, batchSize, syncPeriod, delta, aggType):
    ...
    def getLearner(self):
        ...
        learner = KerasNN(batchSize=self.batchSize, syncPeriod=self.syncPeriod, delta=self.delta, session=session,aggType=self.aggType)
        learner.setCore(nn)
        return learner
